...
##########Footprint analysis
parser.add_option("--proximal",dest="proximal_identify", default="on",help = 'Whether to identify proximal(TSS)-HChub, can choose "on" or "off". Default: on.')
parser.add_option("--distal",dest="distal_identify", default="on",help = 'Whether to identify distal-HChub,can choose "on" or "off". Default: on.')
parser.add_option("--c_proximal", dest="cut_off_proximal", default="10",help = 'The threshold of footrint score and footprint number to identify proximal(TSS)-HChub. {10,15,20}. Default: 10.')
parser.add_option("--c_distal", dest="cut_off_distal", default="10",help = "The threshold of footrint score and footprint number to identify distal-HChub. {10,15,20}. Default: 10.")
...
###################idnetify HChub
now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
print(now_time+" Begain to identify HChub\n",file=log_file1)
def identify_super_peak (ATAC_peak,footprint,cut_off,out_path) :
    ...
    subprocess.check_call("awk '$2>= "+number+' {print $1 '+ '"\t"'+ "$2}' "+ out_path+"/"+options.output_name+"_broadPeak_footprint_number | sed 's/-/"+r'\t'+"/g' > "+ out_path+ "/"+ options.output_name +"_HChub_ATAC_broad_peak.bed" ,shell=True)
    subprocess.check_call("awk '$2< "+number+' {print $1 '+ '"\t"'+ "$2}' "+ out_path+"/"+options.output_name+"_broadPeak_footprint_number | sed 's/-/"+r'\t'+"/g' > "+ out_path+ "/"+ options.output_name +"_MChub_ATAC_broad_peak.bed" ,shell=True)
    subprocess.check_call("bedtools intersect -a "+ ATAC_peak+" -b " + footprint_score_filter+ " -wa -v |awk '{print $1 "+ ' "\t" $2'+'"\t" $3 "\t"'+" 0} ' |cat >> " + out_path+ "/"+ options.output_name +"_MChub_ATAC_broad_peak.bed",shell=True)
    return
####distal
if options.distal_identify == "on" : 
    ...
    now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(now_time+" Skip identifying distal HChub \n",file=log_file1)
####proximal
...
now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
print(now_time+" Identifying of HChub complete\n",file=log_file1)
